...
# Filename patterns
# CHANGE (date/file detection fix): accept BOTH of these:
#   Metrics_YYYY-MM-DD_HH-MM-SS.log.csv   (old)
#   Metrics_YYYY-MM-DD_HH-MM-SS.csv       (new)
# same for Events_... .txt
METRICS_RE = re.compile(
    r"^Metrics_(\d{4}-\d{2}-\d{2})_\d{2}-\d{2}-\d{2}(?:\.log)?\.csv$",
    re.IGNORECASE
)
EVENTS_RE  = re.compile(
    r"^Events_(\d{4}-\d{2}-\d{2})_\d{2}-\d{2}-\d{2}(?:\.log)?\.txt$",
    re.IGNORECASE
)
...
def date_range_from_selection(
    ...
    # ---- Custom full date override ----
    if from_date_str and to_date_str:
        try:
            start = datetime.strptime(from_date_str.strip(), "%m/%d/%Y")
            end = datetime.strptime(to_date_str.strip(), "%m/%d/%Y") + timedelta(days=1)
            if end <= start:
